...
const SubmitManuscriptPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const formData = new FormData();
      formData.append("file", paperFile);
      formData.append("fileStage", "2"); // Submission file
      formData.append("name", paperFile.name);
      formData.append("genreId", "1"); // Usually 'Article Text'
      ...
      // STEP 4: ADD AUTHORS & SET PRIMARY CONTACT
      setStatus("Step 4/4: Adding author details...");
      for (let i = 0; i < authors.length; i++) {
        const author = authors[i];
        const authorPayload = {
          givenName: { en_US: author.name.trim() },
          familyName: { en_US: "" },
          email: author.email.trim(),
          country: "IN",
          userGroupId: OJS_AUTHOR_GROUP_ID,
        ...
        // ✅ Assign first author as the main contact (so emails go to them)
        if (i === 0) {
          await axios.put(
          ...
          console.log("✅ Assigned author as primary contact");
        }
      }
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            {uniqueCode && (
              <p className="mt-4 text-green-700 font-semibold">
                ...
                <br />
                <span className="text-gray-700 text-sm">
                  Please copy this — it’s your official Submission ID for tracking
                  your paper.
                </span>
              </p>
            )}
