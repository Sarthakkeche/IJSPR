...
import React, { useState, useEffect } from "react";
import axios from "axios";
import { FaFileAlt, FaDownload, FaExclamationTriangle, FaSpinner, FaTrash } from "react-icons/fa";
...
export default function PendingManuscripts() {
  ...
  const handleDelete = async (manuscriptId) => {
    ...
    if (!window.confirm("Are you sure you want to permanently delete this manuscript?")) {
      return;
    }

    try {
      // 2. Send DELETE request to the API
      const response = await axios.delete(`${API_BASE_URL}/api/manuscripts/${manuscriptId}`);

      if (response.data.success) {
        // 3. If successful, remove the manuscript from the list in the UI
        setManuscripts(prevManuscripts => prevManuscripts.filter(m => m._id !== manuscriptId));
        alert("Manuscript deleted successfully!");
      } else {
        alert(`Error deleting manuscript: ${response.data.message}`);
      }
    } catch (err) {
      console.error("❌ Failed to delete manuscript:", err);
      alert("An error occurred while deleting the manuscript. Please check the console.");
    }
  };
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {manuscripts.map((m) => (
                ...
                >
                      ...
                      {Object.keys(m)
                        ...
                        .map((key) => {
                          ...
                          if (key === "_id") {
                            return (
                              <p key={key} className="text-xs text-slate-500">
                                <span className="font-medium text-slate-300">ID:</span> {value}
                              </p>
                            );
                          }
                          if (["paperTitle", "title", "name"].includes(key)) return null;
                          if ((key.toLowerCase().includes("date") || key.toLowerCase().includes("at")) && value) {
                            ...
                            if (!Number.isNaN(d.getTime())) {
                              return (
                                <p key={key} className="text-sm text-slate-400">
                                  <span className="font-medium text-slate-200">{prettyKey(key)}:</span> {d.toLocaleString()}
                                </p>
                              );
                            }
                          }
                          if (isFileKey(key)) {
                            return (
                              <p key={key} className="text-sm text-slate-400">
                                <span className="font-medium text-slate-200">{prettyKey(key)}:</span> {typeof value === "string" ? value.split("/").pop() : String(value)}
                              </p>
                            );
                          }
                          return (
                            <p key={key} className="text-sm text-slate-400">
                              <span className="font-medium text-slate-200">{prettyKey(key)}:</span> {value === null || value === undefined ? (
                                <i className="text-slate-500">N/A</i>
                              ) : typeof value === "object" ? (
                                <pre className="text-xs text-slate-300">{JSON.stringify(value)}</pre>
                              ) : (
                  ...
                  {/* --- NEW: Actions section with Download and Delete --- */}
                  <div className="mt-6 pt-4 border-t border-slate-700 flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-2 px-3 py-2 text-sm font-semibold text-red-400 bg-red-900/50 rounded-lg hover:bg-red-800/50 hover:text-red-300 transition-colors"
                    >
                      <FaTrash />
                      <span>Delete</span>
                    </button>
